...
for ii = 1:round(N/2)
    ...
    SNR= 4 + (8-4)*rand(1,1); %choose the SNR (range from 3 to 8)
    Noise=noises(:,:,r)./SNR;
...
for ii=1:round(N/6)
    a = 0.00005 + (0.0005-0.00005)*rand(1,1);% (0.05-0.005）
...
% 添加脉冲噪声
for ii = 1:round(N/6)
    prob = 0.00005 + (0.0005-0.00005)*rand(1,1);% 脉冲出现的概率(0.005-0.0005)
    noise1 = rand(1001, 36);
...
for ii = 1:round(N/6)
    ...
    amp = 0.005 + (0.1-0.005)*rand(1,1); %正弦波振幅(range from 0.05 to 0.3)
